...
func (tc *TokenCreate) check() error {
	if _, ok := tc.mutation.AccessToken(); !ok {
		return &ValidationError{Name: "accessToken", err: errors.New(`ent: missing required field "Token.accessToken"`)}
	}
	if _, ok := tc.mutation.RefreshToken(); !ok {
		return &ValidationError{Name: "refreshToken", err: errors.New(`ent: missing required field "Token.refreshToken"`)}
	}
	if _, ok := tc.mutation.CreatedAt(); !ok {
